...
$(document).ready(function(e) {
	...
	if(typeof(EventSource) !== "undefined") {
		var source = new EventSource("/v1/get/gender/status");
			...
			$.ajax({
				...
				success: function(res) {
					if(res.error == 0) {
						...
						for(i in res.data) {
							var obj = res.data[i];
							var myvar = '<button class="check-star btn-menu sub-cat-btn" data-id="'+obj.CATEGORY_ID+'" style="height: 45px;border: 1px solid #A88C35;background-color: #f4d164;color: #002323;padding: 0px 0px 2px 13px;font-size: 17px;"><span><i class="fa fa-star-o" aria-hidden="true"></i></span> '+obj.CATEGORY_NM+'</button>';
                ...
                if(res.error == 0) {
                    ...
                    for(i in res.data) {
                        ...
                        if(obj.CATEGORY_SUB_ID == null) {
                            ...
                            var myvar = '<div class="group group_'+obj.CATEGORY_SUB_ID+'" data-parent="'+obj.CATEGORY_PARENT_ID+'" data-group-id="'+obj.CATEGORY_SUB_ID+'" style="/* height: 45px; */color: #f4d164;font-weight: bold;font-size: 17px;padding: 7px 0px 0px 0px;">'+
                            '<p class="cat-name" style="font-weight: bold;font-size: 17px;color: #f4d164;width: 15em;">'+obj.CATEGORY_NM+'</p>'+
                            '<div></div>'+
                            '</div>';
                        }
			...
			success: function(res) {
				...
				if(res.error == 0) {
					...
					// updateQuantity($, zeroPad(res.data, 2));
				 	...
				 	$.ajax({
						// url:base_url+'cart/add/'+iid+'/'+zeroPad(quantity, 2),
