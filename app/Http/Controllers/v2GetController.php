...
class v2GetController extends Controller
{
    ...
    public function getShop(Request $request) {
        ...
        return view('v2welcome',$this->return_json);
    }
    ...
    public function checkPendingBillOut() {
        ...
        if(session()->exists('shop_id')) {
            ...
            $check_billout = DB::table('TBL_SERVICE')
            ->where('ITEM_ID','000000000000')
            ->get();

            if($check_billout->count() == 0) {
                $this->return_json['error'] = 0;
            }
		...
		/* if(count($orders) > 0){
			...
			foreach($orders as $order){
				$item = DB::table('TBL_ORDER')
					...
					->where('TBL_ORDER.ITEM_ID', '=', $order->ITEM_ID)
					// ->where('TBL_ORDER.SEQ', '=', $order->SEQ)
					// ->where('TBL_ORDER.SALES_NO', '=', $order->SALES_NO)
