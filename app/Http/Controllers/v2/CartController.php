				...
				foreach($carts as $cart) {
					...
					$note = $cart['note'];
						...
						$items = DB::table('MST_ITEM')
							...
							->select('*')
							->where('MST_ITEM_DTL.ITEM_ID', '=', $item_id)
							->first();
						...
						if(count($items) > 0){
							...
							if($items->TYPE != 0){
								...
								$TBL_SERVICE->ITEM_ID		= $item_id;
								$TBL_SERVICE->ITEM_NM		= $items->ITEM_NM;
								$TBL_SERVICE->ITEM_QTY		= $quantity;
								...
								$arr['total'] = $items->ITEM_PRICE * $quantity;
								$arr['seq'] = $seqOrder;
	...
	public function placeOrderFood(Request $request){
		...
		if(!isset($json_data['data'])){
			...
			foreach($json_data['data'] as $data){
				...
				$data['GENDER_FLG'] = $gender_flg;
				$data['PRINT_FLG'] = $print_flg;
				$data['PRINT_DATE'] = $print_date;
				
				$selectQuery = DB::table('TBL_ORDER')
